...
def getDistance_BetweenTwoOpenDate(pred, reard):
    _isOpen = isOpen(pred)
    if pred == reard:
        return 0
    if pred > reard:
        return None
    if pred is None or reard is None:
        return None
    count = 0
    while True:
        pred = nextOpenDate(pred, 1)
        if pred >= reard:
            if _isOpen is False:
                return count + 1
            return count
...
def getRealTime_Prices(codes):
    # code_price_rel = {}
    # #get_k_data 数据未更新，测试替代方案
    # if isOpen(getLastestOpenDate()) and getHMS() > '15:00:00':
    #     ld = preOpenDate(getLastestOpenDate(), 1)
    #     for code in codes:
    #         df = ts.get_k_data(code, start=ld, end=ld)
    #         price = float(df['close'][df.index.values[0]])
    #         code = df['code'][df.index.values[0]]
    #         code_price_rel.setdefault(code, price)
    #         print("getRealTime_Prices Code:" + code)
